...
HRESULT Stage1Scene::InitObject()
{
	...
	{
		MessageBox(g_hWnd, TEXT("Stage1Scene tile Failed."), TEXT("����"), MB_OK);
		return E_FAIL;
	}
...
void Stage1Scene::TestCode()
{
	...
	if (KeyManager::GetInstance()->IsOnceKeyDown(VK_F4))
		SceneManager::GetInstance()->ChangeScene("Boss", "�ε�_1");
}
...
void Stage1Scene::LoadObject()
{
	...
	if (hFile == INVALID_HANDLE_VALUE)
	{
		MessageBox(g_hWnd, L"LoadObject Failed.", TEXT("���"), MB_OK);
		return;
	}
